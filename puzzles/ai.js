...
/* -------------------------------
        AI ATTEMPTS MOVE
  ------------------------------- */
function opponentAttemptMove(move) {
    ...
    function pickRandMove() {
        moveSet = positions[opponentPosition.shortName].validMoves;
        move = moveSet[getRandomInt(0,moveSet.length-1)];
    }

    if (move == "random" || move == undefined) {
        pickRandMove()
    }
    ...
    AImove = convertMoveNameToObect(move);
